...
;(function($, window, document, undefined) {
	...
	Tree.defaults = {
		...
		// Event handler triggered upon initialization
	...
	Tree.prototype = {
		...
		// Triggers the nodeUnselected event
		_triggerNodeUnselectedEvent: function(node) {

			this.$element.trigger('nodeUnselected', [$.extend(true, {}, node)]);
		},

		_triggerInitializedEvent: function(tree) {
			this.$element.trigger('initialized', [tree]);
		},
		...
		_addSelectedNode: function(node) {
			...
			var index;
			if( (index = this.selectedNodes.indexOf( node )) !== -1 ) {
				this._triggerNodeUnselectedEvent(node);
		...
		_isSelected: function(node) {
			...
			var ret = this.selectedNodes.indexOf(node) !== -1;
			return ret;
		},
		...
		_render: function() {
			...
			if (!self.initialized) {
				...
				self._triggerInitializedEvent(this);
			}
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				var treeItem = $(self._template.item)
					...
					.addClass((self._isSelected(node)) ? 'node-selected': '')
		...
		_buildStyleOverride: function(node) {
			...
			var style = '';
			if (this.options.highlightSelected && (this._isSelected(node))) {
